...
assert_type	: /* empty */		{ $$ = ASSERT_ERROR; }
		| T_POP_WARN comma	{ $$ = ASSERT_WARN; }
		| T_POP_FAIL comma	{ $$ = ASSERT_ERROR; }
		| T_POP_FATAL comma	{ $$ = ASSERT_FATAL; }
;
...
assert		: T_POP_ASSERT assert_type relocexpr
		...
		| T_POP_ASSERT assert_type relocexpr comma string
		{
			if (rpn_isKnown(&$3) && $3.nVal == 0) {
				switch ($2) {
					case ASSERT_FATAL:
		...
		| T_POP_STATIC_ASSERT assert_type const comma string
		{
			if ($3 == 0) {
				switch ($2) {
					case ASSERT_FATAL:
